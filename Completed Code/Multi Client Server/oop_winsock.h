...
public:
	...
	void send_mode(int); // loop that keeps checking if there is a message in message_buffer
	void rec_mode(int); //  loop that keeps receiving messages from clients, and copy these messages to a global variable called char message_buffer
...
public:
	...
	void get_messages(); // loop that keeps calling the receive_message() method and printing any received messages on the terminal
