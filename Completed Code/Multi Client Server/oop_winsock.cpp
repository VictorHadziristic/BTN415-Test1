...
void winsock_server::send_mode(int socket){
	while (true) {
		if (buffer_full) {
			if (strcmp(message_buffer, "quit") == 0) {
				closesocket(connection_socket[socket]);
				this->active_sockets[socket] = 0;
				break;
			}
			else {
				this->send_message(message_buffer, socket);
...
void winsock_server::rec_mode(int socket){
	while (true) {
		memset(this->rx_buffer,0,128);
		strcpy(message_buffer, this->receive_message(socket));
		buffer_full = true;
		if (strcmp(message_buffer, "quit") == 0) {
			closesocket(connection_socket[socket]);
			this->active_sockets[socket] = 0;
...
void winsock_client::get_messages(){
	char rx_buffer[128] = {};
	while (true) {
		strcpy(rx_buffer, this->receive_message());
		std::string r(rx_buffer, strlen(rx_buffer));
		Print(r);
		memset(rx_buffer, 0, 128);
	}
}
